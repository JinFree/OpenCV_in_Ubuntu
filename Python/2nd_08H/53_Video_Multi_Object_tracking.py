...
print(fourcc)
out = cv2.VideoWriter('output_Multiple_Object_Tracker.avi', fourcc, fps, (width, height), True)
    ...
    if ret:
        ...
        if ret:
            for i, newbox in enumerate(bbox):
                ...
                cv2.rectangle(output, p1, p2, colors[i], 2, 1)
        else :
            # Tracking failure
            cv2.putText(output, "Tracking failure detected", (100,80), cv2.FONT_HERSHEY_SIMPLEX, 0.75,(0,0,255),2)
        # Display tracker type on frame
        cv2.putText(output, trackerType + " Tracker", (100,20), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (50,170,50),2);
