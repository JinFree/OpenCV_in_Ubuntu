...
void rangeColor(Mat &image, Mat &result, Scalar &min, Scalar &max, int flag);
...
int lineFittingForPerspectiveImage(Mat &image, Mat &result, vector<Vec4i> &lines, Scalar color = Scalar(0, 0, 255), int thickness = 3, float slope_threshold = (30.0 * PI / 180.0));
