...
def Video(openpath, savepath = "output.avi"):
    ...
    while cap.isOpened():
        ...
        if ret:
            ...
            output = OpenCV.imageProcessing(frame)
...
def splitTwoSideLinesMultiRoi(lines, middle_x, slope_threshold = (5. * np.pi / 180.)):
    lefts = []
    rights = []
    thresh_for_left = middle_x + middle_x * 0.2
    thresh_for_right = middle_x - middle_x * 0.2
    for line in lines:
        x1 = line[0,0]
        y1 = line[0,1]
        x2 = line[0,2]
        ...
        if abs(slope) < slope_threshold:
            continue
        if slope <= 0 and x1 <= thresh_for_left and x2 <= thresh_for_left:
            lefts.append([slope, x1, y1, x2, y2])
        elif slope >= 0 and x1 >= thresh_for_right and x2 >= thresh_for_right:
            rights.append([slope, x1, y1, x2, y2])
    return lefts, rights

def lineFittingMultiRoi(image, lines, roi, color = (0,0,255), thickness = 3, slope_threshold = (5. * np.pi / 180.)):
    result = imageCopy(image)
    height = image.shape[0]
    middle_x = int(0.5*image.shape[1])
    lefts, rights = splitTwoSideLinesMultiRoi(lines, middle_x, slope_threshold)
    left = medianPoint(lefts)
    right = medianPoint(rights)
    min_y = roi[0][0][1]
    max_y = height
    ...
    if left is not None and right is not None:
        ...
        temp = makeBlackImage(result, True)
        cv2.fillPoly(temp, points, (0, 255, 0))
        result = addWeightedImage(result, 100, temp, 20)
    return result
